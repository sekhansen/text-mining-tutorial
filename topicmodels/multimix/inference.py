	...
	def E_step(self):
		...
		for i in range(self.N):
			for k in range(self.K):
				temp_probs[i,k] = np.log(self.rho[k]) + np.dot(self.feature_counts[i,:],np.log(self.mu[k,:]))
	...
	def M_step(self):
		...
		for k in range(self.K):
			for m in range(self.M):
				temp_prob = np.dot(self.type_prob[:,k],self.feature_counts[:,m])
				if temp_prob < 1e-99: temp_prob = 1e-99
				self.mu[k,m] = temp_prob / np.dot(self.type_prob[:,k],self.observations)
