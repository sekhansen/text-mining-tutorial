...
class BOW():
	...
	def pos_count(self,items):
		...
		if items=="tokens":
			...
			pos_stems = [PorterStemmer().stem(t) for t in bow_data.pos_dict.intersection(set(self.token_key.keys()))]
	...
	def neg_count(self,items):
		...
		if items=="tokens":
			...
			neg_stems = [PorterStemmer().stem(t) for t in bow_data.neg_dict.intersection(set(self.token_key.keys()))]
	...
	def uncertain_count(self,items):
		...
		if items=="tokens":
			...
			uncertain_stems = [PorterStemmer().stem(t) for t in bow_data.uncertain_dict.intersection(set(self.token_key.keys()))]
